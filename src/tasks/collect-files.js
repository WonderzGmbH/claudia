  ...
  const logger = optionalLogger || new NullLogger(),
    ...
    installDependencies = function (targetDir) {
      if (useLocalDependencies) {
        ...
        return runNpm(
          targetDir,
          ['install', '-q', '--no-audit', '--omit=dev'].concat(npmOptions),
          logger,
          runQuietly
        );
      }
    ...
    isRelativeDependency = function (dependency) {
      return (
        ...
        (dependency.startsWith('file:') || dependency.startsWith('.') || dependency.startsWith('/'))
      );
    },
          ...
          activeRemapPromise[actualPath] = readjson(path.join(actualPath, 'package.json'))
            .then((packageConf) => {
              ...
              return cleanCopyToDir(actualPath)
                ...
                .then((cleanCopyPath) =>
                  packProjectToTar(cleanCopyPath, workingDir, npmOptions, logger)
                );
            })
        ...
        .then((packageConfig) => {
          if (hasRelativeDependencies(packageConfig)) {
            ...
            return Promise.all(
              ...
              .then(() =>
                fsPromise.writeFileAsync(confPath, JSON.stringify(packageConfig, null, 2), 'utf8')
              )
