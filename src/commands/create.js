...
const addPolicy = require('../tasks/add-policy');
...
const deployProxyApi = require('../tasks/deploy-proxy-api');
...
const lambdaNameSanitize = require('../util/lambda-name-sanitize');
...
const executorPolicy = require('../policies/lambda-executor-policy');
const loggingPolicy = require('../policies/logging-policy');
const vpcPolicy = require('../policies/vpc-policy');
const snsPublishPolicy = require('../policies/sns-publish-policy');
const isSNSArn = require('../util/is-sns-arn');
const lambdaInvocationPolicy = require('../policies/lambda-invocation-policy');
...
module.exports = function create(options, optionalLogger) {
  ...
  const logger = optionalLogger || new NullLogger(),
    ...
    iam = loggingWrap(new aws.IAM({ region: options.region }), {
      log: logger.logApiCall,
      logName: 'iam',
    }),
    lambda = loggingWrap(new aws.Lambda({ region: options.region }), {
      log: logger.logApiCall,
      logName: 'lambda',
    }),
    s3 = loggingWrap(new aws.S3({ region: options.region, signatureVersion: 'v4' }), {
      log: logger.logApiCall,
      logName: 's3',
    }),
    ...
    apiGatewayPromise = retriableWrap(
      loggingWrap(new aws.APIGateway({ region: options.region }), {
        log: logger.logApiCall,
        logName: 'apigateway',
      }),
    ...
    getPackageInfo = function () {
      ...
      return readjson(path.join(source, 'package.json')).then((jsonConfig) => {
        const name = options.name || lambdaNameSanitize(jsonConfig.name),
          description =
            options.description || (jsonConfig.description && jsonConfig.description.trim());
        if (!name) {
          return Promise.reject(
            'project name is missing. please specify with --name or in package.json'
          );
        }
        return {
          name: name,
        ...
        () => {
          ...
          return lambda
            .createFunction({
              ...
              VpcConfig: options['security-group-ids'] &&
                options['subnet-ids'] && {
                  SecurityGroupIds:
                    options['security-group-ids'] && options['security-group-ids'].split(','),
    ...
    waitForState = function (lambdaData) {
      ...
      return waitUntilNotPending(lambda, lambdaData.FunctionName, awsDelay, awsRetries).then(
        () => lambdaData
      );
    },
    ...
    saveConfig = function (lambdaMetaData) {
      ...
      if (lambdaMetaData.api) {
        config.api = {
          id: lambdaMetaData.api.id,
          module: lambdaMetaData.api.module,
        };
      }
      return fsPromise
        .writeFileAsync(configFile, JSON.stringify(config, null, 2), 'utf8')
        .then(() => lambdaMetaData);
    },
  ...
  return initEnvVarsFromOptions(options)
    ...
    .then(() =>
      lambdaCode(s3, packageArchive, options['use-s3-bucket'], options['s3-sse'], options['s3-key'])
    )
    .then((functionCode) => {
...
module.exports.doc = {
  ...
  args: [
    ...
    },
    {
      ...
      optional: true,
      description:
        'The main module to use when creating Web APIs. \n' +
    ...
    },
    {
      ...
      optional: true,
      description:
        'If specified, a proxy API will be created for the Lambda \n' +
    ...
    },
    {
      ...
      optional: true,
      description:
        'A comma-delimited list of binary-media-types to \n' +
    ...
    },
    {
      ...
      optional: true,
      description:
        'A directory or file pattern for additional IAM policies\n' +
    ...
    },
    {
      ...
      optional: true,
      description:
        'The name or ARN of an existing role to assign to the function. \n' +
    ...
    },
    {
      ...
      example: 'sg-1234abcd',
      description:
        'A comma-delimited list of AWS VPC Security Group IDs, which the function will be able to access.\n' +
    ...
    },
    {
      ...
      description:
        'A comma-delimited list of AWS VPC Subnet IDs, which this function should be able to access.\n' +
