...
const cleanUpPackage = require('../tasks/clean-up-package');
const runPostCollectScript = require('../tasks/run-post-collect-script');
const aws = require('aws-sdk');
const allowApiInvocation = require('../tasks/allow-api-invocation');
const lambdaCode = require('../tasks/lambda-code');
const markAlias = require('../tasks/mark-alias');
const retriableWrap = require('../util/retriable-wrap');
const rebuildWebApi = require('../tasks/rebuild-web-api');
const validatePackage = require('../tasks/validate-package');
const apiGWUrl = require('../util/apigw-url');
...
const updateEnvVars = require('../tasks/update-env-vars');
const getOwnerInfo = require('../tasks/get-owner-info');
const fs = require('fs');
const fsPromise = require('../util/fs-promise');
const fsUtil = require('../util/fs-util');
const loadConfig = require('../util/loadconfig');
const isSNSArn = require('../util/is-sns-arn');
const snsPublishPolicy = require('../policies/sns-publish-policy');
const retry = require('oh-no-i-insist');
const waitUntilNotPending = require('../tasks/wait-until-not-pending');
const combineLists = require('../util/combine-lists');
...
module.exports = function update(options, optionalLogger) {
  ...
  const logger = optionalLogger ?? new NullLogger(),
    awsDelay =
      (options && options['aws-delay'] && parseInt(options['aws-delay'], 10)) ||
    ...
    handlerFileExtension = options.extension ?? 'js',
    ...
    updateConfiguration = function (newHandler) {
      ...
      if (options['add-layers'] || options['remove-layers']) {
        configurationPatch.Layers = combineLists(
          functionConfig.Layers && functionConfig.Layers.map((l) => l.Arn),
          options['add-layers'],
    ...
    validateOptions = function () {
      ...
      if (options['optional-dependencies'] === false && options['use-local-dependencies']) {
        return Promise.reject(
          'incompatible arguments --use-local-dependencies and --no-optional-dependencies'
        );
      }
      ...
      if (options.memory || options.memory === 0) {
        if (options.memory < limits.LAMBDA.MEMORY.MIN) {
          return Promise.reject(
            `the memory value provided must be greater than or equal to ${limits.LAMBDA.MEMORY.MIN}`
          );
        }
        if (options.memory > limits.LAMBDA.MEMORY.MAX) {
          return Promise.reject(
            `the memory value provided must be less than or equal to ${limits.LAMBDA.MEMORY.MAX}`
          );
        }
  ...
  return validateOptions()
    ...
    .then((config) => {
      ...
      lambda = loggingWrap(new aws.Lambda({ region: lambdaConfig.region }), {
        log: logger.logApiCall,
        logName: 'lambda',
      });
      s3 = loggingWrap(new aws.S3({ region: lambdaConfig.region, signatureVersion: 'v4' }), {
        log: logger.logApiCall,
        logName: 's3',
      });
      iam = loggingWrap(new aws.IAM({ region: lambdaConfig.region }), {
        log: logger.logApiCall,
        logName: 'iam',
      });
      apiGateway = retriableWrap(
        loggingWrap(new aws.APIGateway({ region: lambdaConfig.region }), {
          log: logger.logApiCall,
          logName: 'apigateway',
        }),
    ...
    .then(() => {
      return updateEnvVars(
        ...
        lambda,
        lambdaConfig.name,
        functionConfig.Environment && functionConfig.Environment.Variables
      );
    })
    .then(() => {
...
module.exports.doc = {
  ...
  args: [
    ...
    },
    {
      ...
      optional: true,
      description:
        'The function execution time, in seconds, at which AWS Lambda should terminate the function',
    },
    {
      argument: 'runtime',
      optional: true,
      description:
        'Node.js runtime to use. For supported values, see\n http://docs.aws.amazon.com/lambda/latest/dg/API_CreateFunction.html',
    },
    {
      argument: 'memory',
      optional: true,
    ...
    },
    {
      ...
      optional: true,
      description:
        'Specifies the desired architecture, either x86_64 or arm64. \n' +
    ...
    },
    {
      ...
      optional: true,
      description:
        'Any additional options to pass on to NPM when installing packages. Check https://docs.npmjs.com/cli/install for more information',
    ...
    },
    {
      ...
      description:
        'Name of the stage variable for storing the current API configuration signature.\n' +
    ...
    },
    {
      ...
      description:
        'the name of a NPM script to execute custom processing after claudia finished packaging your files.\n' +
    ...
    },
    {
      ...
      example: 'customNpmScript',
      description:
        'the name of a NPM script to execute custom processing after claudia finished collecting your files.\n' +
        'Note that development dependencies are not available at this point, but you can use npm uninstall to remove utility tools as part of this step.',
      since: '5.0.0',
    },
    {
      ...
      optional: true,
      description:
        'keep the produced package archive on disk for troubleshooting purposes.\n' +
    ...
    },
    {
      ...
      description:
        'The name of a S3 bucket that Claudia will use to upload the function code before installing in Lambda.\n' +
    ...
    },
    {
      ...
      description:
        'The key to which the function code will be uploaded in the s3 bucket referenced in `--use-s3-bucket`',
    },
    {
      argument: 's3-sse',
      optional: true,
      example: 'AES256',
      description:
        'The type of Server Side Encryption applied to the S3 bucket referenced in `--use-s3-bucket`',
    },
    {
      argument: 'update-env',
      optional: true,
      ...
      description:
        'comma-separated list of VAR=VALUE environment variables to set, merging with old variables',
    },
    {
      argument: 'set-env',
      optional: true,
      example: 'S3BUCKET=testbucket,SNSQUEUE=testqueue',
      description:
        'comma-separated list of VAR=VALUE environment variables to set. replaces the whole set, removing old variables.',
    },
    {
      ...
      description:
        'file path to a JSON file containing environment variables to set, merging with old variables',
    },

    {
      ...
      example: 'production-env.json',
      description:
        'file path to a JSON file containing environment variables to set. replaces the whole set, removing old variables.',
    },
    {
    ...
    },
    {
      ...
      optional: true,
      description:
        'A comma-delimited list of Lambda layers to attach to this function. Setting this during an update replaces all previous layers.',
    ...
    },
    {
      ...
      optional: true,
      description:
        'A comma-delimited list of additional Lambda layers to attach to this function. Setting this during an update leaves old layers in place, and just adds new layers.',
      example: 'arn:aws:lambda:us-east-1:12345678:layer:ffmpeg:4',
    },
    {
      ...
      optional: true,
      description:
        'A comma-delimited list of Lambda layers to remove from this function. It will not remove any layers apart from the ones specified in the argument.',
      example: 'arn:aws:lambda:us-east-1:12345678:layer:ffmpeg:4',
    },
    {
      argument: 'dlq-sns',
